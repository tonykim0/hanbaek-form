 ...
 * 여기는 차수가 축이고, 저기는 상대·회차가 축이다. 한 화면에서 토글로 바꾸면
 * 두 화면 중 어디를 보고 있었는지가 주소에 안 남아서 링크로 보낼 수도, 돌아올 수도 없다.
 *
 * ★한백 전용★ 저장소가 한백이 아니면 빈 목록을 준다. 여기서도 한 번 더 막는다 —
 * 화면만 막으면 저장소를 바꿀 때 가드가 빠진다.
 *
 * 열람 전용도 본다. 이 화면은 목록을 그리는 것이 전부라 따로 잠글 것이 없다 —
 * 넣는 자리(준공마감일 지정·차수별 수금 기록)는 현장 상세의 기성 탭에 있다(2026-08-28).
 * 보는 자리와 넣는 자리를 가른 이유: 수금은 현장 하나를 놓고 「이 차수를 받았나」를 보며
 * 적는 일이고, 이 표는 전 현장의 차수 상태를 훑는 자리다.
 */
  ...
  return (
    <>
      ...
      {/*
        ...
        * 현장 상세의 시공 탭에서 넣고, 준공마감일은 한백이 지정한다. 수금 체크와 준공마감일
        * 지정은 아직 없다. 이 세 문장이 화면 아래 붙어 있었는데, 넣는 자리가 여기 없다는 것은
        * 넣는 칸이 없는 것으로 이미 보인다(화면 규칙 2번). 글자를 지우고 빈 <p> 만 남아
        * 있던 것을 마저 걷었다.
        */}
    </>
  );
