 ...
 *
 * 관리자 계정은 여기서 만들 수 없다. 화면에서 만들 수 있게 두면 실수 한 번으로
 * 협력사가 원가·마진을 보는 계정이 생긴다 — 그건 배포 설정(AUTH_USERS)으로만 늘린다.
 */
...
export async function PATCH(request: Request) {
  ...
  let body: { id?: string; active?: unknown };
  try {
    body = (await request.json()) as typeof body;
  } catch {
    return NextResponse.json({ error: '요청을 읽을 수 없습니다.' }, { status: 400 });
  ...
  if (!body.id || typeof body.active !== 'boolean') {
    return NextResponse.json({ error: 'id 와 active 가 필요합니다.' }, { status: 400 });
  }
