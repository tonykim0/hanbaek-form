 ...
 * 배치 하나를 다룬다. [한백 전용]
 *
 *   PATCH  { org, kind, from, to }      지급일 이동 — 지급 줄 전부 + 세금계산서가 한 트랜잭션
 *   DELETE { org, kind, payDate }       가확정 취소 — 지급 줄 전부 삭제, 회차는 지급 가능으로
 *
 * 항목 하나만 빼는 것은 여기가 아니라 원장 삭제다(DELETE /api/projects/[id]/payouts).
 * 확정된 배치는 둘 다 거부된다 — 해제부터.
 */
...
export const DELETE = adminWrite<
  Record<string, never>,
  { org?: unknown; kind?: unknown; payDate?: unknown }
>('한백 관리자만 가확정을 취소할 수 있습니다.', async ({ body, actor }) => {
  if (typeof body?.org !== 'string' || !body.org.trim()) throw new BadRequest('지급처가 없습니다.');
  if (!PAYOUT_KINDS.includes(body.kind as PayoutKind)) {
    throw new BadRequest('구분(영업비/시공비)이 없습니다.');
  }
  ...
  return getRepository().cancelPayoutBatch(body.org.trim(), body.kind as PayoutKind, body.payDate, actor);
});
