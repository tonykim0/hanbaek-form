...
/* ── 운영사별 원자 칸 그리드 ──────────────────────────────────────────────
 * 줄 = 교체유형 × 수전방식, 칸 = 연수 × 건축물유형(6). 케이스 한 행이 여러 칸을 덮는
 * 블록이라, 행 목록만 봐서는 어느 칸이 비었는지 알 수 없다 — 칸으로 펴서 보인다.
 * 줄 수는 운영사마다 다르다: 한전불입이 붙는 것은 환경부 신규뿐이고(powerTypesOfRepl),
 * 자체투자 신규위치가 없는 운영사도 있다(replTypesOf). 있을 수 없는 조합에는 줄을 안 만든다.
 *
