...
export interface ProjectRepository {
  ...
  /**
   * 자기가 남긴 진행현황을 지운다 (한백 지시 2026-08-30).
   *
   * ★예전에는 지우는 길을 두지 않았다.★ 기록이니 남겨야 한다는 판단이었는데, 실제로는
   * 잘못 눌러 남긴 빈 줄과 딴 현장에 적은 글이 영영 남았다 — 고치기로는 지울 수 없다
   * (빈 내용은 저장이 막힌다).
   *
   * ★자기가 쓴 것만이다★ — 고치기와 같은 잣대다. 남의 글을 지울 수 있으면 기록이 아니라
   * 편집물이 된다. 한백도 남의 글은 못 지운다.
   *
   * ★지운 글은 감사기록에 통째로 남는다★ — 화면에서 사라지는 것과 없던 일이 되는 것은
   * 다르다(지급 기록 삭제와 같은 판단).
   */
  deleteNote(input: { projectId: string; noteId: string }, actor: Actor): Promise<void>;
   ...
   * 영업비 2차=준공완료, 시공비 1차=설치완료, 시공비 2차=준공완료. 영업비 지급조건 서류,
