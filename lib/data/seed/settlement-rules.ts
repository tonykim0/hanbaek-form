/**
 * 정산 규칙 — 단가와 분리된 별도 테이블. 불변(추가·비활성만).
 *
 * 왜 금액을 행마다 적지 않고 규칙으로 두는가:
 * 매트릭스 28행을 검산해보니 정산 「모양」은 6가지뿐이고 금액은 턴키에서 유도된다.
...
export const SETTLEMENT_RULES: SettlementRule[] = [
  {
    id: 'pl-2step',
    name: '플러그링크 — 환경부 승인 20만 → 준공마감 잔액',
    steps: [
      { trigger: '환경부 승인', basis: { kind: '고정', unit: 200_000 } },
      { trigger: '준공마감', basis: { kind: '잔액' } },
    ],
    note: '2차 금액칸에 배포가의 절반이 적혀 있으나 유형이 잔액이라 계산으로 대체된다. 한백 최종확인 대기.',
    active: true,
  },
  {
    id: 'nice-2step',
    name: '나이스인프라 — 착공 110만 → 준공마감 잔액',
    steps: [
  ...
  {
    id: 'hec-3step',
    name: '현대엔지니어링(상) — 환경부 승인 30만 → 착공 80만 → 준공마감 120만',
    steps: [
