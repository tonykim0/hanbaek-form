...
/**
 * 파일명이 분명히 말하는 서류 — ★AI 분류보다 앞선다.★
 *
 * 판독은 비결정적이라 같은 서류가 날마다 다른 칸에 들어갈 수 있다. 이름이 분명한 것은
 * 이름으로 못 박는다: 고치는 자리가 하나이고, 두 번 돌려도 같은 답이고, 테스트가 된다.
 * 프롬프트로 「이건 이 칸이 아니다」라고 타이르는 것은 그 다음이다.
 *
 * ★전기차 등록대수 확인 공문★ (한백 2026-08-28) — 단지에 전기차가 몇 대 등록돼 있는지
 * 묻고 받는 공문이다. 그런데 「기설치 충전기 설치이력」과 글자가 겹친다(대수 · 확인 ·
 * 표 모양) — 현대엔지니어링 접수에서 자꾸 그 칸으로 들어갔다. 남의 칸에 앉으면 두 번
 * 나쁘다: 그 칸은 환경부 필수라 진짜 설치이력이 밀려나고(같은 칸이면 하나만 남는다),
 * 공문은 검수에서 반려되어 다시 올라온다.
 *
