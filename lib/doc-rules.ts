...
const SPECS: DocSpec[] = [
  ...
  {
    /*
     * 설치승낙서 — 건물 쪽이 「여기에 설치해도 좋다」고 적어 주는 글 (한백 지시 2026-09-02).
     *
     * ★선택이다.★ 어느 현장에서 요구되는지 아직 정해지지 않았다 — 필수로 잘못 걸면 그
     * 현장의 접수가 영구히 막힌다(이 파일이 여러 번 겪은 실수다). 받는 자리를 먼저 만들고,
     * 요구해야 하는 조건이 확인되면 req 를 좁힌다.
     *
     * ★설치승인서(현대엔지니어링)와 다른 서류다★ — 한 글자 차이라 사람도 판독기도 헷갈린다.
     * 승인서는 운영사가 우리에게 내주는 것이고, 승낙서는 건물 쪽이 내주는 것이다.
     * 이름 규칙(categoryFromFileName)으로 「승낙」을 못 박아 판독이 흔들려도 갈리게 했다.
     */
    key: 'installConsent',
    name: '설치승낙서',
    req: () => 'o',
  },
  {
    ...
    name: '견적서 (SK)',
    /*
     * SK일렉링크 현장은 전부 낸다 (한백 지시 2026-09-03 — 자체투자만이던 것을 SK 전체로
     * 넓혔다). SK 정책이 시공 전 설치도면·견적서를 서면 승인받게 하고, 승인되지 않은
     * 시공비는 청구할 수 없다(lib/pricing-policy-sk-h2.ts). 그래서 접수 때 받아 둔다.
     *
     * 필수는 접수만 막는 게 아니다 — 영업비 지급조건(payoutDocsMissing)에도 든다.
     * 이미 계약이 끝난 SK 현장도 견적서가 없으면 영업비 지급이 이 칸에 걸린다.
     */
    req: (c) => (c.cpo === 'SK일렉링크' ? 'm' : 'o'),
  },
];
...
export const PROCESS_DOCS = [
  ...
  /*
   * 발주 때 같이 받는다 (한백 지시 2026-08-31).
   * ★키를 'quote' 로 두지 않는다★ — 계약 서류에 그 키가 이미 있고(견적서 (SK 자체투자)),
   * 두 표를 가르는 판정이 ★이름이 겹치지 않는다★는 전제 위에 서 있다(isProcessDocKind).
   * 겹치면 SK 계약 견적서가 공정 표로 들어가 계약 칸에서 사라진다.
   */
  { key: 'orderQuote', name: '견적서' },
  /*
  ...
  /*
   * ★설치 상자에서 발주 상자로 옮겼다★ (한백 지시 2026-08-31) — 신고는 설치 전에 하는
   * 일이라 설치완료 묶음에 있을 자리가 아니었다.
   */
  { key: 'installNotice', name: '충전시설 설치 신고서' },
  /* 착공 전에 갖추는 안전서류 묶음 — 한 칸에 여러 장이 붙는다(한백 지시 2026-08-31) */
  { key: 'preStartDocs', name: '착공 전 서류 (TBM일지 · 상시위험성평가 등)' },
