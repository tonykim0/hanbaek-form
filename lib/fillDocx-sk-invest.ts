/**
 * SK일렉링크 자체투자 docx template filler.
 * template_invest.docx 사용 (계약서 + 사전현장컨설팅 결과서).
 */

import JSZip from 'jszip';
import {
  SkInvestFormData,
  buildSkInvestSdtMaps,
  buildSkInvestTextReplacements,
  buildSkInvestHeaderTableMap,
} from './schema-sk-invest';
...
function fillTextSdt(sdt: Element, value: string): boolean {
  ...
  if (sdtPrList.length > 0) {
    ...
    while (showings.length > 0) showings[0].parentNode?.removeChild(showings[0]);
  }
...
function toggleCheckboxSdt(sdt: Element, checked: boolean): boolean {
  ...
  for (let i = 0; i < runs.length; i++) {
    ...
    if (fonts.length > 0) {
      const f = fonts[0];
      for (const a of ['asciiTheme', 'hAnsiTheme', 'eastAsiaTheme', 'cstheme']) {
...
function fillEmptyCell(doc: Document, cell: Element, value: string): boolean {
  ...
  const texts = paras[0].getElementsByTagNameNS(W_NS, 't');
  if (texts.length > 0) {
    texts[0].textContent = value;
    texts[0].setAttributeNS(XML_NS, 'xml:space', 'preserve');
...
export async function fillSkInvestTemplate(form: SkInvestFormData): Promise<FillResult> {
  const response = await fetch('/sk/template_invest.docx');
  if (!response.ok) {
  ...
  const { text: textMap, checkbox: cbMap } = buildSkInvestSdtMaps(form);
  ...
  return { blob, filledSdtText: sdtTextFilled, filledSdtCheckbox: sdtCbFilled, filledTextReplace: textReplaceFilled, filledHeaderCells, totalSdt: sdts.length, unmatchedIds };
}
