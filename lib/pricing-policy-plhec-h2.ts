 ...
 *   자체투자 교체        공동 7년 20+200=220 · 10년 20+220=240 (신설)
 *                        ★상업 10년(120만)은 걷었다 (한백 2026-09-05) — 「플러그링크 자투
 *                        상업은 그냥 없는 걸로 하자」. migrations/0055 가 그 케이스를 지운다.
 * 분해는 같은 프레임(마진 20 · 시공 100 고정, 나머지 영업).
 *
 * ★자투 상업 10년은 케이스를 두지 않는다 (한백 2026-09-05).★ 문서에 금액(120만)은 있었지만
 * 시공 95 + 마진 20 을 빼면 영업비가 0 이라(0046 이 남는 5만을 마진으로 옮겼다) 분해가
 * 서지 않았다. 기성 1차 20만은 문서상 「영업비」인데 그 자리에 영업비가 없는 케이스가 된다 —
 * 값이 앞뒤가 안 맞는 케이스를 목록에 두면 언젠가 그것으로 계약이 들어온다. 라인 0건일 때
 * 걷는다. 필요해지면 분해를 정하고 새로 만든다.
 *
 ...
 * ★끝도 적는다 — 「2026년 7월 1일 ~ 8월 31일」 (한백 2026-09-06).★ 9월 1일부터는 다른 정책이라
 * 이 벌은 8월 31일 접수분까지다. 시작일만 적혀 있으면 열려 있는 것처럼 읽힌다. 정렬·시기 탭은
 * 앞 날짜를 읽으므로(lib/pricing-match startKey) 그대로 하반기에 선다. 상반기(1월 20일)는 그대로.
 * 반영은 migrations/0057 — 이름의 괄호도 같이 바뀐다(이름은 적용 시작에서 유도된다).
 *
...
const PL_INV_ROWS: { term: number; bldg: '공동주택' | '상업시설'; total: number }[] = [
  ...
  // 상업 10년(120만)은 걷었다 (한백 2026-09-05) — 위 머리 주석 참조. migrations/0055
];
