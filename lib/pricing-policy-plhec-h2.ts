...
/**
 * 사업구분이 조건을 가른다 — ★자체투자는 한전불입이 없다★ (한백 2026-08-29).
 *
 * 그 줄을 전 케이스에 붙이지 않는다: 환경부 한전불입 케이스에 「한전불입 불가」가 적히면
 * 「이 케이스는 한전불입인데 불가라고?」가 된다. 조건은 그 케이스에 맞는 말만 적는다
 * (플러그링크 상업시설을 가른 것과 같은 규칙).
 *
 * ★현대엔지니어링은 연동 사업을 하지 않는다 (한백 2026-08-29).★ 그래서 연동 케이스가
 * 없고, 여기에 연동 가지도 두지 않는다 — 쓰지 않는 가지를 남겨 두면 다음 사람이
 * 「연동도 되는구나」로 읽고 케이스를 만든다. 플러그링크는 연동을 한다(7년 55만·10년 75만).
 */
function hecInstall(bizType: string): string {
  return [
    ...HEC_INSTALL_COMMON,
    ...(bizType === '자체투자' ? ['· 모자분리만 (한전불입 불가)'] : []),
  ].join('\n');
}
