...
export function canChangeContractDocs(
   ...
   *
   * ★이 검사가 아래 착공 검사보다 앞에 온다★ — 확인이 없다는 것은 계약이 다시 열렸다는
   * 뜻이고(deriveStage 가 intake 로 유도해 현장이 계약 보드로 내려간다), 그때 협력사가
   * 고치는 것이 맞다. 착공 뒤라도 한백이 「확인 취소」를 누르면 그렇게 된다 — 이 줄이
   * 뒤에 있으면 계약보완 칸에 선 현장을 협력사가 못 고치는 새 교착이 생긴다(반박 검토
   * 2026-09-05 에서 걸림). 착공 뒤 반려는 확인을 지우지 않으므로(applyReviewSideEffects)
   * 보통은 아래 착공 검사가 그대로 문을 닫는다.
   */
  if (!contractConfirmed) return true;
  /*
   * ★착공 뒤에는 협력사가 계약 서류를 못 바꾼다★ (한백 지시 2026-09-05 「착공 뒤에는
   * 협력사 수정 없이 우리만 수정할 수 있고, 필요하면 서류만 따로 받아서 올리는 방식으로」).
   *
   * 그 전에 한 번 다른 길을 갔다(2026-09-04 아침): 착공 뒤 반려된 칸을 협력사가 다시
   * 올릴 수 있게 「반려된 칸」 예외를 넣었다. 그런데 그 예외는 첫 장을 넣는 순간 사라져
   * (칸이 「제출됨」이 되므로) 두 장짜리 스캔이 반쪽으로 잠겼다(감사 M12). 예외를 더
   * 정교하게 만드는 대신 한백이 문을 닫기로 했다 — 공사가 도는 현장의 서류는 한백이
   * 받아서 올린다. 빈 칸 예외도 여기서 같이 닫힌다 — 착공 전 구간에서만 산다.
   */
  if (statusIndex(status) >= statusIndex(PARTNER_DOCS_CLOSED_AT)) return false;
  if (slotEmpty) return true;
  return statusIndex(status) < statusIndex(CONTRACT_DOCS_LOCK_AT);
}

/**
 * 협력사의 계약 서류 손이 완전히 닫히는 자리 — ★착공★ (한백 지시 2026-09-05).
 *
 * 운영사 제출(CONTRACT_DOCS_LOCK_AT)부터 여기까지는 예외가 있다(확인이 풀린 칸·빈 칸).
 * 여기서부터는 없다 — 공사가 도는 현장의 서류는 한백이 받아서 올린다. 반려도 협력사에게
 * 공을 넘기지 않는다(store/docs applyReviewSideEffects).
 */
export const PARTNER_DOCS_CLOSED_AT: ProcessStatus = '착공';

/**
 * 못 바꾸는 이유 — 저장소와 화면이 같은 문장을 쓴다. 구간에 따라 말이 다르다:
 * 착공 전에는 「빈 칸에는 올릴 수 있다」가 참이고, 착공 뒤에는 「한백에 보내라」가 답이다.
 * 한 문장으로 뭉치면 협력사가 안 되는 길을 시도하거나(착공 뒤 빈 칸) 되는 길을 모른다.
 */
export function contractDocsLockedWhy(status: ProcessStatus): string {
  return statusIndex(status) >= statusIndex(PARTNER_DOCS_CLOSED_AT)
    ? '착공 뒤로는 협력사가 계약 서류를 바꿀 수 없습니다 — 고칠 서류가 있으면 한백에 파일을 보내주세요. 한백이 올립니다.'
