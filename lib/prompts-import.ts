...
const FIELD_GUIDE: Array<[field: string, where: string]> = [
  ...
  ['powerSharingKw/powerSharingQty/powerSharingCableQty', '별지5호·별지7호 전력분배형 충전기의 kW·기수·케이블 개수. 각각 숫자만. 비어 있으면 ""'],
...
const CHECKBOX_GUIDE: Array<[label: string, mapping: string]> = [
  ...
  ['5번 전기 수용 용량', '고압시 변압기 용량 「확인」→highVoltageConfirmed / 저압 계약전력 「확인」→lowVoltageConfirmed. 4번 전력인입의 모자분할·한전불입과 별개로 각각 판독'],
  ['6번 중복설치', '급속충전기→dupFast+dupFastQty / 완속충전기→dupSlow+dupSlowQty / 전력분배형→dupDist+dupDistQty / 과금형콘센트→dupOutlet+dupOutletQty / 키오스크→dupKiosk+dupKioskQty / 「해당사항 없음」→dupNone. 빈 체크박스는 false이며, dupNone이 체크돼도 개별 항목은 보이는 그대로 판독'],
];
...
5. **뒤집히거나 돌아간 페이지도 판독하세요.** 협력사 스캔본은 페이지가 90°·180° 회전된 채
   스캔된 경우가 흔합니다. 글자가 거꾸로 보여도 그 페이지를 앞 서류의 연속으로 넘기거나
   버리지 말고, 머릿속에서 바로 돌려 읽어 어떤 서식인지(detectedDocs)와 적힌 값을 판독하세요.
   회전된 페이지가 있으면 issues에 「N페이지 스캔이 회전되어 있습니다」를 남기세요.
...

{
  ...
  "fields": {
    ...
    "powerMoja": true,
    "powerHanjeon": false,
    "highVoltageConfirmed": true,
    "lowVoltageConfirmed": false,
