-- 병행·기타 칸 (2026-08-23) — 설치조건에 뭉쳐 있던 병행·교체 조건을 제 행으로 가른다.
-- 매트릭스가 이 칸들을 행으로 편다. 전부 nullable — 옛 케이스는 「아직 안 적음」이 맞다.

alter table pricing_rules
  add column if not exists coexist_terms text,
  add column if not exists misc_terms    text;
...
update pricing_rules set
  ...
  other_support       = '열화상 3면당 1대 무상 (옥내·지하 한정) · 전기안전점검 수수료는 한백 수령 — 하도급 미지급(턴키금액 포함)',
  coexist_terms       = '나이스 단독은 일부 병행 가능(사전 협의) · 타사 혼합은 병행 불가',
  misc_terms          = '교체공사는 노후설비에 따른 일부 재시공 필수 — 분전함~충전기 케이블·배관 신설, 차단기·튜브 교체, 도색(레터링). 배관이 후강전선관·덕트면 재사용 가능 · 교체 전 입주민 의향조사(민원 사전 차단) · 타CPO 교체는 계약종료 확인 — 해지 내용증명·소유권, 보조금 의무운영 5년 경과',
  note                = null
