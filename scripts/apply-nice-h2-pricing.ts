 ...
 * 옮겼는지의 근거도 그 파일에 있다. 이 스크립트는 그것을 DB 에 돌리는 껍데기다.
 *
 * 프로덕션에는 2026-08-22 에 이미 들어갔다 — 접속 문자열이 Vercel 에서 Sensitive 라
 * 로컬에서 붙을 길이 없어, 관리자가 화면에서 한 번 누르는 임시 라우트로 넣고 그 라우트와
 * 단추는 걷어냈다. 다시 넣을 일이 생기면 그때 같은 방식을 다시 만든다 —
 * 한 번 쓰는 자리를 남겨두면 정책이 바뀐 뒤에도 이 안의 낡은 표가 돈다.
 *
