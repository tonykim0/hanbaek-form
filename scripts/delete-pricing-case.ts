 ...
 *
 * ★이제 화면에도 지우는 자리가 있다★ (2026-09-04 — /pricing 케이스 줄의 「삭제」,
 * 저장소 deletePricingRule). 참조가 있으면 화면·저장소·이 스크립트 모두 거절한다 —
 * 그 현장의 지급액·기성을 계산할 수 없게 되기 때문이다. 그쪽 정규 경로는 「개정·중지」다.
 * 이 스크립트는 여러 건을 한 번에 걷을 때 남겨 둔다.
 *
