...
describe('advanceBlockers — 진행 단추를 막는 것들', () => {
  it('행위신고: 아무것도 없으면 대상 여부부터 고르라고 한다', () => {
    expect(advanceBlockers('충전기 발주', 'notifyDoneAt', P()))
      .toEqual(['대상 여부를 먼저 고르세요', '환경부 승인일']);
  });

  it('행위신고: 대상을 고르면 신고일·파일을 묻는다', () => {
    expect(advanceBlockers('충전기 발주', 'notifyDoneAt', P({ notifyRequiredAt: '2026-08-27' })))
      .toEqual(['행위신고일', '신고 파일', '환경부 승인일']);
  });

  it('행위신고: 대상 아님 + 환경부 승인일이면 열린다', () => {
    expect(advanceBlockers('충전기 발주', 'notifySkippedAt',
      P({ notifySkippedAt: '2026-08-27', envApprovalDate: '2026-08-01' }))).toEqual([]);
  });

  it('★발주: 다섯 칸을 다 묻는다 — 모뎀 발주 수량이 빠졌던 자리다★', () => {
    expect(advanceBlockers('충전기 수령', null, P())).toEqual([
...
describe('canEnter — 서버가 막는 자리', () => {
  ...
  it('조건이 다 차면 다음 칸이 열린다', () => {
    const p = P({ status: '행위신고', envApprovalDate: 'd', notifySkippedAt: 'd' });
    expect(canEnter('충전기 발주', p)).toEqual({ ok: true });
  });

  it('막힌 이유는 사람이 읽는 말이다', () => {
    const r = canEnter('충전기 발주', P({ status: '행위신고' }));
    expect(r.ok).toBe(false);
