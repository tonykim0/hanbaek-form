...
export const PROCESS_STATUSES = [
  ...
  '착공',
  /*
   * ★「설치완료」였다 — 칸 이름을 그 안에서 하는 일로 바꿨다★ (한백 지시 2026-09-04).
   *
   * 설치가 끝났다는 것은 이 칸에 ★들어오는 조건★이고(설치완료일·설치완료 사진), 들어와서
   * 하는 일은 통신·개통이다 — 이 칸이 담는 상자도 「개통」 하나뿐이다. 상태를 칸 이름으로
   * 쓰면 보드에 「설치완료」라는 칸이 서서, 거기 있는 현장이 무엇을 기다리는지 안 보인다.
   *
  ...
  '개통 및 통신확인',
  /*
   * ★「개통완료」 칸을 걷었다★ (한백 지시 2026-08-31). 통신완료일·개통완료일은 앞 칸의
   * 개통 상자가 이미 받는다 — 그것을 적고 나서 「개통완료」라는 칸에 한 번 더 서는
...
export interface ProjectSummary {
  ...
  rejectedDocs: number;
  /**
   * 그중 파일이 없는 칸 — 아직 안 낸 것이다(「누락 서류 보완요청」이 세웠다).
   * 할 일 문구가 「미제출 N건 제출」과 「반려 N건 보완」을 가르는 데 쓴다.
   */
...
export interface ContractState {
  ...
  rejected: number;
  /**
   * 그중 파일이 한 장도 없는 칸 — 「누락 서류 보완요청」이 세운 자리다.
   *
   * ★안 낸 것과 퇴짜 맞은 것을 가르는 값이다.★ 둘 다 status='rejected' 라 rejected 로는
   * 못 가른다. 협력사가 할 일이 다르므로(내라 / 고쳐서 다시 내라) 문구도 갈려야 한다.
   */
