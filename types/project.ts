 ...
 *
 * ★현장 쪽은 막지 않는다.★ 아파트가 한전불입금을 직접 내겠다고 해서 자체투자로 진행한
 * 예가 실제로 있다(전남 무안 전남개발공사 · 충북 청주 율량동 현대아파트). 그런 현장은
 * 한백이 모자분리 케이스를 손으로 지정한다 — 단가 축에 없는 조합이라 라인이 자동으로는
 * 안 맞고, 그래서 「막힌 라인」에 서서 눈에 띈다. 그게 맞는 동작이다.
 */
